---
title: "(5.4,5.5) ETS Experiments and Hierarchical Normal Models"
author: "Weihang Ren"
output: 
  beamer_presentation:
    includes:
...
plot1 <- ggplot(table,aes(x=school,y=y,colour=school))+geom_point()+
    ...
    labs(title = "Coaching Effect with 95% CI")
```
...
\begin{enumerate}
    ...
    \item There is a different solution for Hierarchical Normal Model $$p(\mu,\tau|y)\propto p(\mu,\tau)p(y|\mu,\tau)$$
    \begin{itemize}
